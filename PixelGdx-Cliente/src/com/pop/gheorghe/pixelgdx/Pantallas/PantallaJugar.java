...
//         Serpientes
        for(MapObject object : map.getLayers().get(6).getObjects().getByType(RectangleMapObject.class)){
        	Rectangle rect = ((RectangleMapObject) object).getRectangle();
        	serpientes.add( new Serpiente(this, rect.getX() / PixelGdx.PPM, rect.getY() / PixelGdx.PPM));
        }
	}
	...
	public void update( float delta ) {
		...
		// TODO: SERVER Conseguir información del jugador 2 en el server
		if ( game.isServer ) {
...
//			gameCam.position.x = Math.round( (gamePort.getScreenWidth()/PixelGdx.PPM)/2 * 100f)/100f; // Esto soluciona el problema de las líneas al renderizar
...
//			gameCam.position.x = Math.round( jugador.b2body.getPosition().x * 100f)/100f ; // Esto soluciona el problema de las líneas al renderizar
		...
		else if ( jugadorPrincipal.b2body.getPosition().x > 31.5f - mitadViewPortWidth )
